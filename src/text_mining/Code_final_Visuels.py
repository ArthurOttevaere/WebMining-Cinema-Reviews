...
def run_text_mining(df, show_plots=False):
    ...
    df["compound"] = df["eval_words"].apply(
        lambda toks: sia.polarity_scores(" ".join(toks))["compound"])  # compute compound score
    ...
    wc = WordCloud(width=800, height=400, background_color="white").generate_from_frequencies(
        token_counter)  # generate word cloud
    plt.figure(figsize=(12, 6))
    ...
    if "review_score" in df.columns:
        ...
        for bar, pct in zip(bars, percentages):
            plt.text(bar.get_x() + bar.get_width() / 2, pct + 0.5, f"{pct:.1f}%", ha="center",
                     va="bottom")  # annotate bars
